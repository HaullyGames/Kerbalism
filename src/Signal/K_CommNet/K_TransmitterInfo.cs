...
namespace K_CommNet
{
  public sealed class K_TransmitterInfo
  {
    public static float rangeModifier;   //	CommNet Range Modifier
    ...
    public short cntAnt;                                                                                          //	NEEDS TO REMOVE: DEBUG ONLY

    public List<ModuleDataTransmitter> ListAntenna = new List<ModuleDataTransmitter>();                           //	Antennas list

    public K_TransmitterInfo(CommNode node)
    {
      ...
      bool extended;                                                                                              //	Fixed and deployable antennas
      ...
      string antennaT = null;                                                                                     //	Temporary antenna for compare
      string relayT = null;                                                                                       //	Temporary relay for compare
      ...
      {
        Vessel v = KERBALISM.Lib.CommNodeToVessel(node);
        if (v != null)
        {
          if (v.loaded)
          {
            cntAnt = 0;
            if (v.FindPartModulesImplementing<ModuleDataTransmitter>().Count > 0)
            {
              foreach (ModuleDataTransmitter antenna in v.FindPartModulesImplementing<ModuleDataTransmitter>())
              {
                ...
                extended = (anim == null || anim.deployState == ModuleDeployablePart.DeployState.EXTENDED);

                //	Calc range only for Antennas that is working
                if (extended && antenna.isEnabled)
                {
                  cntAnt++;
              ...
              AntennaRange = mainAntenna * Math.Pow((AntennaSum / mainAntenna), AntennaCombExponent);
              RelayRange = mainRelay * Math.Pow((RelaySum / mainRelay), RelayCombExponent);
            }
          }
          else
          {
            //	for each part
            cntAnt = 0;
            foreach (ProtoPartSnapshot partSnap in v.protoVessel.protoPartSnapshots)
            {
              //	get part prefab (required for module properties)
              Part part_prefab = PartLoader.getPartInfoByName(partSnap.partName).partPrefab;
              ...
              foreach (ProtoPartModuleSnapshot module in partSnap.modules)
              {
                //	I'm only insterested in Transmitter
                if (module.moduleName != "ModuleDataTransmitter") continue;

                extended = antenna.CanCommUnloaded(module);
                //	Get data only if the module is enable & extended(fixed is equal extended)
                if (extended && KERBALISM.Lib.Proto.GetBool(module, "isEnabled"))
                {
                  cntAnt += 1;
                  ListAntenna.Add(antenna);
            ...
            foreach (ModuleDataTransmitter antenna in ListAntenna)
            {
              ...
              //	Have to confirm, I put to arround up
              //	Antenna Power X CommNet rangeModifier
              AntennaXModified = Math.Round(antenna.CommPower * rangeModifier, 1);
              ...
              //	Average Weighted Combinability
              if (antenna.CommCombinable) AntennaComb += Math.Round(AntennaXModified * ((allAntennaEqual) ? 1 : antenna.antennaCombinableExponent), 1);
